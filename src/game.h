...
)
{
    for (int width = width_start; width < width_end; width++)
    {
        for (int length = length_start; length < length_end; length++)
        {
            ...
            *to_fill = empty_cell;
            to_fill->type = type;
            sprintf(to_fill->name, NAME_FORMAT, floor, width, length);
...
void fix_cell_neighbour(cell* current_cell)
{
    ...
    sscanf(current_cell->name, NAME_FORMAT, &floor, &width, &length);
    for (direction d = NORTH; d < DIRECTION_COUNT; d++)
    {
        // -1 NORTH, +1 SOUTH +1 EAST -1 WEST
        int neighbour_width = width - (d == NORTH) + (d == SOUTH);
        int neighbour_length = length + (d == EAST) - (d == WEST);
        ...
        cell *neighbour = maze[floor][neighbour_width][neighbour_length];
